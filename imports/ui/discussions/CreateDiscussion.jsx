...
export const CreateDiscussion = ({ toggleModal }) => {
  ...
  const createDiscussionTour = [
    {
      ...
      content:
        "Give your discussion a title like 'Should Badgers be welcomed into polite Rabbit society?'" +
  ...
  const submitDiscussion = () => {
    ...
    if (
      ...
      (groupName.length !== 0 || groupId.length !== 0)
    ) {
      let newGroupId;
  ...
  const insertDiscussion = (newGroupId) => {
    Meteor.call(
      ...
      description,
      [categoryId],
      discussionTemplate._id,
      (error, result) => {
        Meteor.call(
          "discussions.insert",
          result,
          groupId.length !== 0 ? groupId : newGroupId,
          timeLimit,
          isHui,
          true
        );
  ...
  return (
    <Modal open={true} closeOnDimmerClick={false} size="small">
      ...
      <Modal.Header className={"newDiscussion1"}>
        Create A Discussion
      </Modal.Header>
      <Modal.Content>
        <Form>
          ...
          {createNewGroup && (
            <div>
              ...
              {errGroupName ? (
                <div
                  style={{ height: "10px", color: "red", marginBottom: "10px" }}
                >
                  {errGroupName}
                </div>
              ) : (
