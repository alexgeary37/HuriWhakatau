...
export const RatingComponent = () => {
    ...
    if (questionType) {
      Meteor.call(
        ...
        (err, fetchedExperimentArray) => {
          ...
          let exptRatings = experiment.ratings.filter(
            (rating) => rating.rating !== ""
          );
          // Select a random rating from the experiment's rating set
          rating =
            exptRatings[
              Math.floor(Math.random() * Math.floor(exptRatings.length))
            ];
          // find the appropriate response set
          responses = responseSet.find(
            (set) => set.responseType === rating.responseType
          );
          ...
          indices[ratingScale].forEach((index) =>
            labelSet.push(responses.fullRange[index])
          );
          header = rating.rating;
          // get a random comment for a discussion in the experiment.
          Meteor.call(
            "comments.getRandomExperimentCommentForRating",
      ...
      Meteor.call(
        ...
        (err, fetchedQuestionArray) => {
          ...
          responses = responseSet.find(
            (set) => set.responseType === question.item.responseType
          );
          responseIndices = indices[ratingScale];
          // get the appropriate labels given the number of responses set eg 2, 5 or 7
  ...
  const addAnswerValue = (value) => {
    ...
    let score = questionItem.reverse
      ? ratingLabels.reverse().indexOf(value)
      : ratingLabels.indexOf(value);
  ...
  const submitAnswer = () => {
    ...
    if (answerString) {
      if (typeOfQuestion) {
        Meteor.call(
          "commentRatings.addRating",
          questionItem.itemId,
          questionItem.experimentId,
          {
      ...
      <ListItem key={Math.random()}>
        ...
        >
          {questionItem.headerText ? (
            <List.Header as={"h4"} content={questionItem.headerText} />
          ) : (
            <Placeholder>
              <Placeholder.Line
                length={"full"}
          ...
          {questionItem.bodyText && (
            <List.Description as={Segment} content={questionItem.bodyText} />
          )}
          <hr />
          {questionItem.ratingLabels.length > 0 ? (
