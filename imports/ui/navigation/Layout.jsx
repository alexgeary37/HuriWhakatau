import {
  ...
  export const Layout = ({ page, isDiscussion }) => {
    ...
    const {
      ...
      if (currentUser?.profile?.friendList) {
        ...
        if (fetchedFriends[0] !== undefined) {
          friendsOnline = fetchedFriends.some(
            (friend) => friend.status.online === true
          );
        }
        ...
        if (fetchedGroupMembers[0] !== undefined) {
          groupMembersOnline = fetchedGroupMembers.some(
            (member) => member.status.online === true
          );
        }
      }
      return {
    ...
    const searchFriendsComponent = () => {
      return (
        <div
          style={{ marginLeft: "10px", width: "40vh" }}
        ...
        >
          ...
          <Button onClick={submitFriendSearch} icon labelPosition="right">
            ...
            <Icon
              loading={isSearching}
              name={!isSearching ? "right arrow" : "circle notch"}
            />
          </Button>
    ...
    return (
      ...
      >
        ...
        <Sidebar.Pushable
          as={Segment}
          style={{ height: "90vh", backgroundColor: "rgb(30,30,30)" }}
        >
          <div>
            <Sidebar
              as={Segment}
            ...
            >
              <Icon
                size={"big"}
                name={
                  (showSidebar ? "left" : "right") + " arrow alternate circle"
                }
                style={{
                  marginTop: "-10px",
                  marginLeft: showSidebar ? "220px" : "35px",
                }}
              />
              {/*my friends*/}
              <Menu.Item
                ...
                {showSidebar &&
                  ...
                  pendingFriends.map((pendingFriend) => (
                    <Menu.Item
                      key={
                        pendingFriend._id
                      } /*title={pendingFriend.online ? 'online' : 'offline'}*/
                    >
                    ...
                    >
                      ...
                      >
                        ...
                        <NotificationBadge
                          ...
                          effect={[null, null, null, null]}
                          style={{
                            color: groupMember.status.online
                              ? groupMember.status.idle
                                ? "yellow"
                                : "green"
                              : "red",
                            backgroundColor: groupMember.status.online
                              ? groupMember.status.idle
                                ? "yellow"
                                : "green"
                              : "red",
